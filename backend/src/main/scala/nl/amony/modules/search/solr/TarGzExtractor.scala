...
object TarGzExtractor {
  def extractResourceTarGz(resourcePath: String, targetDirectory: Path): Unit = {
    ...
    if resourceStream == null then { throw new RuntimeException(s"Resource not found: $resourcePath") }

    // Create target directory if it doesn't exist
    Files.createDirectories(targetDirectory)

    Using.resource(new TarArchiveInputStream(new GzipCompressorInputStream(new BufferedInputStream(resourceStream)))) {
      tarIn =>
        ...
        while entry != null do {
          ...
          if entry.isDirectory then { Files.createDirectories(targetPath) }
            ...
            // Extract file
            Using(new FileOutputStream(targetPath.toFile)) {
              output =>
