...
trait ProcessRunner extends Logging {
  ...
  def useProcess[T](cmd: String, args: Seq[String])(fn: fs2.io.process.Process[IO] => IO[T]): IO[T] = fs2.io.process.ProcessBuilder(cmd, args.toList)
    .spawn[IO].use(process => fn(process))
}
