...
//  val files = mutable.ListBuffer.empty[Path]
  ...
  override def preVisitDirectory(dir: Path, attrs: BasicFileAttributes): FileVisitResult =
    if directoryFilter(dir) then FileVisitResult.CONTINUE else FileVisitResult.SKIP_SUBTREE

  override def visitFile(file: Path, attrs: BasicFileAttributes): FileVisitResult =
    if fileNameFilter(file) then files = (file, attrs) +: files
...
object RecursiveFileVisitor extends Logging:
  ...
  def streamFilesInDirectoryRecursive(
    dir: Path,
    directoryFilter: Path => Boolean,
    fileFilter: Path => Boolean
  ): Stream[IO, (Path, BasicFileAttributes)] = fs2.Stream.emits(listFilesInDirectoryRecursive(dir, directoryFilter, fileFilter))
