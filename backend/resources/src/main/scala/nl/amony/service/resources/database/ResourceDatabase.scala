...
class ResourceDatabase(pool: Resource[IO, Session[IO]]) extends Logging:
  ...
  def useTransaction[A](f: (Session[IO], Transaction[IO]) => IO[A]): IO[A] =
    pool.use { s => s.transaction.use { tx => f(s, tx) } }
  ...
  private[database] object tables {
    ...
    object resource_tags {
      ...
      def replaceAll(s: Session[IO], bucketId: String, resourceId: String, tagIds: List[Int]): IO[Unit] = 
        for {
          ...
          _ <- if (tagIds.nonEmpty) s.prepare(Queries.resource_tags.upsert(rows.size)).flatMap(_.execute(rows)) else IO.unit
        } yield ()
    ...
    object tags {
      ...
      def all(s: Session[IO]) =
        s.prepare(Queries.tags.all).flatMap(_.stream(Void, defaultChunkSize).compile.toList)

      def upsert(s: Session[IO], tagLabels: List[String]): IO[Completion] =
        s.prepare(Queries.tags.upsert(tagLabels.size)).flatMap(_.execute(tagLabels.toList))
  ...
  private def updateTagsForResource(s: Session[IO], bucketId: String, resourceId: String, tagLabels: List[String]) =
    for {
      tags <- if (tagLabels.nonEmpty) tables.tags.upsert(s, tagLabels) >> tables.tags.getByLabels(s, tagLabels) else IO.pure(List.empty)
  ...
  def getById(bucketId: String, resourceId: String): IO[Option[ResourceInfo]] =
    useSession: s =>
      s.prepare(Queries.resources.getByIdJoined).flatMap(_.stream((bucketId, resourceId), defaultChunkSize).map(toResource).compile.toList.map(_.headOption))
